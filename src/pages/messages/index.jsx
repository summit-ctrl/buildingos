...
const SEED_CONVOS = [
  { id:'cv1', contactId:'c1', channel:'sms',      unread:2, messages:[
    { id:'m1', from:'them', text:'Hi, the hot water in my unit has stopped working.', ts:'9:14 am' },
    { id:'m2', from:'me',   text:'Hi Aiden, I\'ll arrange a plumber for tomorrow morning 9-11am.', ts:'9:22 am' },
    { id:'m3', from:'them', text:'Just checking — is the plumber still coming today?', ts:'8:50 am', unread:true },
  ]},
  { id:'cv2', contactId:'c2', channel:'whatsapp', unread:1, messages:[
    { id:'m4', from:'them', text:'Hi, I wanted to ask about renewing my lease. It expires in January.', ts:'Yesterday' },
    { id:'m5', from:'me',   text:'Great timing! I\'ll send you the renewal offer this week.', ts:'Yesterday' },
    { id:'m6', from:'them', text:'Sounds good 👍', ts:'Yesterday', unread:true },
  ]},
  { id:'cv3', contactId:'c4', channel:'email',    unread:0, messages:[
    { id:'m7', from:'them', text:'Can you send me the latest levy notice and strata financials?', ts:'Mon' },
    { id:'m8', from:'me',   text:'Hi James, attaching the Q1 levy notice and financials.', ts:'Mon' },
  ]},
]
...
const Badge = ({ label, bg, color }) => <span style={{ background:bg, color, fontSize:11, fontWeight:600, padding:'2px 8px', borderRadius:99, whiteSpace:'nowrap' }}>{label}</span>
...
export default function MessagesPage() {
  ...
  useEffect(()=>{ msgEndRef.current?.scrollIntoView({behavior:'smooth'}) },[activeConvo])
  ...
  function sendMessage() {
    ...
    const msg={id:'m-'+Date.now(),from:'me',text:inputText,ts:new Date().toLocaleTimeString('en-AU',{hour:'2-digit',minute:'2-digit'})}
    ...
    <div className="main-content" style={{padding:0,display:'flex',flexDirection:'column',height:'100%'}}>
      ...
      <div style={{padding:'14px 20px 0',background:'#fff',borderBottom:'1px solid #e5e7eb',flexShrink:0}}>
        ...
        <div style={{display:'flex',gap:2}}>
          {[{id:'inbox',label:'Inbox',badge:totalUnread||null},{id:'broadcast',label:'Broadcast'},{id:'agent',label:'AI Agent',badge:agentEnabled?'On':null,badgeBg:'#dcfce7',badgeColor:'#15803d'}].map(t=>(
            <button key={t.id} onClick={()=>setView(t.id)} style={{padding:'8px 14px',border:'none',background:'none',cursor:'pointer',fontSize:13,fontWeight:500,color:view===t.id?BLUE:'#6b7280',borderBottom:view===t.id?`2px solid ${BLUE}`:'2px solid transparent',marginBottom:-1,display:'flex',alignItems:'center',gap:6}}>
              {t.label}{t.badge&&<span style={{background:t.badgeBg||'#ef4444',color:t.badgeColor||'#fff',fontSize:10,fontWeight:700,padding:'1px 6px',borderRadius:99}}>{t.badge}</span>}
            </button>
          ))}
        </div>
      </div>
          ...
          <div style={{width:280,borderRight:'1px solid #e5e7eb',background:'#fff',display:'flex',flexDirection:'column',flexShrink:0}}>
            ...
            <div style={{flex:1,overflowY:'auto'}}>
              {filteredConvos.map(c=>{
                ...
                return <div key={c.id} onClick={()=>{setActiveConvo(c);setConvos(cs=>cs.map(x=>x.id===c.id?{...x,unread:0}:x))}}
                  ...
                  onMouseEnter={e=>{if(!isActive)e.currentTarget.style.background='#f9fafb'}} onMouseLeave={e=>{if(!isActive)e.currentTarget.style.background='transparent'}}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginTop:2}}>
                      <div style={{fontSize:12,color:'#6b7280',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',maxWidth:160}}>{last?.from==='me'?'You: ':''}{last?.text}</div>
              ...
              <div style={{padding:'12px 16px',borderBottom:'1px solid #e5e7eb',background:'#fff',display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0}}>
                <div style={{display:'flex',alignItems:'center',gap:10}}>
                  ...
                  <div>
                    ...
                    <div style={{fontSize:12,color:'#6b7280',marginTop:1}}>{contact?.unit&&`Unit ${contact.unit} · `}{contact?.phone} · {contact?.email}</div>
                  </div>
                </div>
                <div style={{display:'flex',gap:6,alignItems:'center'}}>
                  ...
                  <div style={{marginLeft:4,fontSize:12,color:'#6b7280'}}>via <strong>{CHANNEL_LABELS[channel]}</strong></div>
                </div>
              </div>
              <div style={{flex:1,overflowY:'auto',padding:'16px',display:'flex',flexDirection:'column',gap:10,background:'#f9fafb'}}>
                {activeConvo.messages.map(m=>{
                  const isMe=m.from==='me'
                  return <div key={m.id} style={{display:'flex',flexDirection:isMe?'row-reverse':'row',gap:8,alignItems:'flex-end'}}>
                    {!isMe&&<Avatar initials={contact?.avatar} bg={contact?.avatarBg} color={contact?.avatarColor} size={28}/>}
                    <div style={{maxWidth:'70%'}}>
                      <div style={{padding:'10px 14px',borderRadius:isMe?'16px 16px 4px 16px':'16px 16px 16px 4px',background:isMe?BLUE:'#fff',color:isMe?'#fff':'#111827',fontSize:13,lineHeight:1.5,boxShadow:'0 1px 2px rgba(0,0,0,0.06)',border:isMe?'none':'1px solid #e5e7eb'}}>{m.text}</div>
              ...
              <div style={{padding:'12px 16px',background:'#fff',borderTop:'1px solid #e5e7eb',flexShrink:0}}>
                {channel==='call'
                  ...
                  : <div style={{display:'flex',gap:8}}>
                      <input value={inputText} onChange={e=>setInputText(e.target.value)} onKeyDown={e=>e.key==='Enter'&&!e.shiftKey&&sendMessage()} placeholder={`Message via ${CHANNEL_LABELS[channel]}…`} style={{flex:1,padding:'10px 14px',borderRadius:8,border:'1px solid #d1d5db',fontSize:13,background:'#f9fafb'}}/>
                      <button onClick={sendMessage} style={{padding:'10px 18px',borderRadius:8,border:'none',background:BLUE,color:'#fff',fontSize:13,fontWeight:500,cursor:'pointer'}}>Send</button>
                    </div>
                }
                <div style={{fontSize:11,color:'#9ca3af',marginTop:6}}>Preferred: {CHANNEL_ICONS[contact?.preferredChannel]} {CHANNEL_LABELS[contact?.preferredChannel]} · Powered by Twilio</div>
              </div>
            </div>
          })()}
        </div>
          ...
          {bcSent
            ...
            : <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:20,maxWidth:900}}>
                <div>
                  ...
                  <div style={{marginBottom:14}}>
                    <div style={{fontSize:11,fontWeight:600,color:'#6b7280',textTransform:'uppercase',letterSpacing:'0.05em',marginBottom:6}}>Message</div>
                  ...
                  <button onClick={sendBroadcast} style={{padding:'8px 16px',borderRadius:6,border:'none',background:BLUE,color:'#fff',fontSize:13,fontWeight:500,cursor:'pointer'}}>{CHANNEL_ICONS[bcChannel]} Send to {bcRecipients.length} recipient{bcRecipients.length!==1?'s':''}</button>
                ...
                <div>
                  ...
                  <div style={{border:'1px solid #e5e7eb',borderRadius:8,overflow:'hidden',background:'#fff'}}>
                    {CONTACTS.map(c=>{
                      ...
                      return <div key={c.id} onClick={()=>setBcRecipients(r=>r.includes(c.id)?r.filter(x=>x!==c.id):[...r,c.id])}
                        ...
                        <Badge label={c.type} bg={c.type==='tenant'?'#dcfce7':c.type==='owner'?'#e8f0fd':'#fef3c7'} color={c.type==='tenant'?'#15803d':c.type==='owner'?BLUE:'#92400e'}/>
                      </div>
                    })}
                  </div>
                </div>
          ...
          <div style={{maxWidth:800}}>
            <div style={{background:'#fff',border:'1px solid #e5e7eb',borderRadius:10,padding:'16px 20px',marginBottom:20,display:'flex',alignItems:'center',justifyContent:'space-between'}}>
              <div>
                ...
                <div style={{fontSize:13,color:'#6b7280'}}>Automatically replies to inbound SMS and WhatsApp enquiries. Escalates to you if it can't answer.</div>
              </div>
              <div style={{display:'flex',alignItems:'center',gap:10}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:20}}>
              <div>
                ...
                <div style={{background:'#f0f9ff',border:'1px solid #bae6fd',borderRadius:10,padding:'14px'}}>
                  <div style={{fontSize:12,fontWeight:600,color:'#0369a1',marginBottom:6}}>🔌 Integrations</div>
                  {[{icon:'💬',name:'Twilio SMS',s:'Ready to connect'},{icon:'📱',name:'Twilio WhatsApp',s:'Ready to connect'},{icon:'📞',name:'Twilio Voice',s:'Coming soon'},{icon:'✉️',name:'SendGrid',s:'Ready to connect'}].map(i=>(
                    <div key={i.name} style={{display:'flex',alignItems:'center',gap:8,padding:'6px 0',borderBottom:'1px solid #e0f2fe'}}>
                      <span style={{fontSize:16}}>{i.icon}</span>
                      <div style={{flex:1}}><div style={{fontSize:12,fontWeight:600}}>{i.name}</div></div>
                      <span style={{fontSize:10,color:i.s.includes('Ready')?BLUE:'#9ca3af',fontWeight:500}}>{i.s}</span>
                    </div>
                  ))}
                </div>
              </div>
              <div style={{background:'#fff',border:'1px solid #e5e7eb',borderRadius:10,padding:'16px'}}>
                ...
                <div style={{borderTop:'1px solid #e5e7eb',paddingTop:10}}>
                  <div style={{fontSize:11,fontWeight:600,color:'#374151',marginBottom:6}}>Add Q&A</div>
